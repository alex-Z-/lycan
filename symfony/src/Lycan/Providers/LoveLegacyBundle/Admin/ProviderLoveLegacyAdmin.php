...
namespace Lycan\Providers\LoveLegacyBundle\Admin;
...
class ProviderLoveLegacyAdmin extends ProviderAdmin
{
	...
	protected function configureFormFields(FormMapper $formMapper)
	{
		...
		$formMapper
			->tab('Credentials')
			->with('LoveLegacy Credentials', array('class' => 'col-md-7'))->end()
		...
		$formMapper
			->tab('Credentials')
			->with('LoveLegacy Credentials')
			->add('nickname', 'text')
				->add('baseUrl', 'url')
				->add('client', 'text', [ 'label' => 'API Key' ])
				->add('secret', 'text', [ 'label' => 'API Secret' ])
			...
			->with('Channel Configuration')
				...
				->add('supportsRealTimePricing', 'checkbox', ['required' => false, 'disabled' => true, 'label' => 'Supports Realtime Pricing'])
				->end()
			->end();
