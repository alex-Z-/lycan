...
use Lycan\Providers\LoveLegacyBundle\Incoming\Hydrator\SchemaHydrator as Hydrator;
use Lycan\Providers\LoveLegacyBundle\Incoming\Transformer\Transformer;
...
class Manager  implements ManagerInterface {
	...
	public function getQueuePullProviderClosure(){
		...
		return function(ProviderLoveLegacyAuth $object, Property $property = null) {
	...
	public function getQueuePullListingsClosure(){
		return function($listings){
			...
			foreach($listings as $index=>$listing){
				...
				try {
					// love Cottages, we have the full Listing Record. So just pass this.
					$msg = [ "id" => $listing['id'], "_full" => $listing, "provider" => $this->getProvider()->getId(), "batch" => (string) $message['batch'], "jobsInBatch" => $jobsInBatch, "jobIndex" => $index + 1 ];
					$logger->info(sprintf("Sending Property with ID of %s to Queue for fetch.", $listing['id']), array_merge( ["input" => $listing], $msg ));
				
					$routingKey = "lycan.provider.pull.listing.lovelegacy";
