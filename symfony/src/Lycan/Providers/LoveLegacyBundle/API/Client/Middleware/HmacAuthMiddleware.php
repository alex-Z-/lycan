...
use Lycan\Providers\LoveLegacyBundle\API\Client\Middleware\Key;

use Lycan\Providers\LoveLegacyBundle\API\Client\Middleware\RequestSigner;
...
class HmacAuthMiddleware
{
	 ...
	 */
	public function __construct(Key $key, $realm = 'LoveLegacy', array $customHeaders = [])
	{
		$this->key = $key;
	...
	public function __invoke(callable $handler)
	{
		return function ($request, array $options) use ($handler) {
			...
			$promise = function (ResponseInterface $response) use ($request) {
				// Unfortunately, LoveLegacy does not have a response authentication.
